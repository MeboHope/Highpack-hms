-- Payment/deposit repair: align payment methods with the Equity workflow and the actual leases schema.
-- The leases table stores the security deposit as `deposit`; there is no `security_deposit` column on leases.
...
BEGIN
  IF EXISTS (
    ...
    WHERE n.nspname = 'public' AND t.relname = 'payments' AND c.conname = 'payments_payment_method_check'
  ) THEN
...
ALTER TABLE public.payments
  ...
  CHECK (payment_method IN ('mpesa','card','bank_transfer','cash','equity','other'));
...
BEGIN
  ...
  SELECT COALESCE(value, '') INTO v_policy
  ...
  -- `leases.deposit` is the authoritative security-deposit amount.
