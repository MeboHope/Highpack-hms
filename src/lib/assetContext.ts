...
export function getAssetContext(assetClass?: string | null, operationModel?: string | null, propertyType?: string | null): AssetContext {
  const asset = assetClass || 'built_property';
  const model = operationModel || 'long_term_rental';
  const type = propertyType || '';
  const landLike = /land|plot|acre|ranch|farm|parcel/i.test(type);
  ...
  if (asset === 'land' || model === 'land_sale' || landLike) return { assetClass: asset, operationModel: model, label: 'Land parcel', relationshipLabel: 'Buyer / leaseholder', primarySubject: 'Parcel', dashboardTitle: 'Your land workspace', financeLabel: 'Purchase & payments', stayOrLeaseLabel: 'Purchase / lease', serviceLabel: 'Documents & due diligence', icon: 'land' };
