      ...
      <aside className="hidden lg:flex w-64 bg-white border-r border-ink-100 flex-col fixed h-screen z-40">
        ...
        <div className="p-4 border-b border-ink-100">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 flex items-center justify-center bg-brand-900 text-white font-bold" style={{ borderRadius: '6px' }}>
        ...
        <div className="p-3 border-t border-ink-100">
          <Link to="/" className="flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-ink-600 hover:bg-ink-50" style={{ borderRadius: '6px', minHeight: '44px' }}>
            <Search className="w-5 h-5" /> View marketplace
          </Link>
          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50" style={{ borderRadius: '6px', minHeight: '44px' }}>
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-64 bg-white flex flex-col border-r border-ink-100">
            <div className="p-4 border-b border-ink-100 flex items-center justify-between">
              <Brand compact />
              <button onClick={() => setMobileOpen(false)} className="flex h-10 w-10 items-center justify-center border border-ink-100 bg-white text-ink-600 hover:bg-ink-50" style={{ borderRadius: '6px' }} aria-label="Close navigation">
                <X className="w-5 h-5" />
              </button>
            </div>
            <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
      ...
      <div className="flex-1 lg:ml-64">
        <header className="sticky top-0 z-30 bg-white border-b border-ink-100">
          <div className="flex items-center justify-between px-4 sm:px-6 h-[64px]">
            <div className="flex items-center gap-3">
              <button className="lg:hidden flex h-10 w-10 items-center justify-center border border-ink-100 bg-white text-ink-700 hover:bg-ink-50" style={{ borderRadius: '6px' }} onClick={() => setMobileOpen(true)} aria-label="Open navigation">
            ...
            <div className="flex items-center gap-2">
              {profile?.role === 'admin' && <button type="button" onClick={() => setGlobalSearchOpen(true)} className="hidden md:flex h-10 min-w-52 items-center justify-between gap-3 border border-ink-200 bg-ink-50 px-3 text-left text-xs text-ink-500 hover:border-ink-300 hover:bg-white" style={{ borderRadius: '6px' }} aria-label="Search"><span className="flex items-center gap-2"><Search className="h-4 w-4" />Search…</span><kbd className="border border-ink-200 bg-white px-1.5 py-0.5 text-[10px] font-semibold text-ink-400" style={{ borderRadius: '6px' }}>Ctrl K</kbd></button>}
              {profile?.role === 'admin' && <button type="button" onClick={() => setGlobalSearchOpen(true)} className="flex h-10 w-10 items-center justify-center border border-ink-100 bg-white text-ink-600 hover:bg-ink-50 md:hidden" style={{ borderRadius: '6px' }} aria-label="Search"><Search className="w-5 h-5" /></button>}
              <Link to="/notifications" className="flex h-10 w-10 items-center justify-center border border-ink-100 bg-white text-ink-600 hover:bg-ink-50" style={{ borderRadius: '6px' }} aria-label="Notifications">
                <Bell className="w-5 h-5" />
              </Link>
              <Link to="/favorites" className="flex h-10 w-10 items-center justify-center border border-ink-100 bg-white text-ink-600 hover:bg-ink-50" style={{ borderRadius: '6px' }} aria-label="Favorites">
        ...
        {globalSearchOpen && profile?.role === 'admin' && (
          <div className="fixed inset-0 z-[60] bg-ink-950/40 p-4" onMouseDown={() => setGlobalSearchOpen(false)}>
            <div className="mx-auto mt-[10vh] w-full max-w-2xl overflow-hidden border border-ink-200 bg-white" style={{ borderRadius: '8px' }} onMouseDown={(e) => e.stopPropagation()}>
              <div className="flex items-center gap-3 border-b border-ink-100 px-4 py-3">
                ...
                <button type="button" onClick={() => setGlobalSearchOpen(false)} className="px-2 py-1 text-xs font-semibold text-ink-400 hover:bg-ink-50" style={{ borderRadius: '6px' }}>Esc</button>
              </div>
              <div className="max-h-[55vh] overflow-y-auto p-2">
                {globalQuery.trim().length < 2 ? (
                  ...
                  globalResults.map((result, index) => (
                    <button key={`${result.type}-${result.title}-${index}`} type="button" onClick={() => openSearchResult(result.to)} className="flex w-full items-center gap-3 px-3 py-3 text-left hover:bg-ink-50" style={{ borderRadius: '6px' }}>
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center bg-ink-50 text-ink-600 border border-ink-100" style={{ borderRadius: '6px' }}><Search className="h-4 w-4" /></span>
        ...
        <main className="p-4 sm:p-6 lg:p-6 max-w-[1200px] mx-auto min-h-[calc(100vh-64px)]">
          <div className="mb-4 flex flex-col gap-2 border border-ink-100 bg-ink-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between" style={{ borderRadius: '8px' }}>
            <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 text-xs text-ink-500">
              {workspaceAsset && <span className="hidden max-w-[280px] truncate bg-white px-2.5 py-1 font-semibold border border-ink-100 sm:inline-flex" style={{ borderRadius: '6px' }}>{workspaceAsset.name}</span>}
              <span className="h-1.5 w-1.5 bg-green-600" style={{ borderRadius: '6px' }} />
