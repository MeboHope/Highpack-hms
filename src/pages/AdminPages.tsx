...
import { Building2, Users, Calendar, Wallet, Home, CheckCircle, XCircle, ShieldCheck, Receipt, UserCheck, Wrench, Search, Download, Eye, RefreshCw, TrendingUp, ArrowUpRight, ArrowDownRight, CalendarClock, UserRound, LogOut, AlertTriangle, Bell, Activity, CheckCheck, MapPin } from 'lucide-react';
...
import { loadDashboardPropertyPerformance, loadManagedExpenses, loadManagedMaintenance, type DashboardPropertyPerformance } from '@/lib/operationalData';
import { getPropertyPresentation } from '@/lib/propertyPresentation';
import type { Property, PropertyUnit, Profile, Reservation, Payment, RentInvoice, SystemSettings, TaxRecord, Notification, Lease } from '@/lib/supabase';
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setSummary(performance);
      const mix: Record<string, { total: number; occupied: number; available: number }> = {};
      performance.filter((row) => getPropertyPresentation(row.assetClass, row.operationModel, row.propertyType).showUnitInventory && getPropertyPresentation(row.assetClass, row.operationModel, row.propertyType).showOccupancy).forEach((row) => Object.entries(row.unitTypes).forEach(([type, total]) => {
  ...
  const totals = summary.reduce((a, r) => { const view = getPropertyPresentation(r.assetClass, r.operationModel, r.propertyType); return { properties: a.properties + 1, units: a.units + (view.showUnitInventory && view.showOccupancy ? r.units : 0), occupied: a.occupied + (view.showOccupancy ? r.occupied : 0), available: a.available + (view.showOccupancy ? r.available : 0), reserved: a.reserved + (view.showOccupancy ? r.reserved : 0), tenants: a.tenants + (view.showTenantMetrics ? r.tenants : 0), rent: a.rent + (view.showRentMetrics ? r.collectedRent : 0), tax: a.tax + r.tax, plots: a.plots + (view.kind === 'land' ? (r.plotCount || 0) : 0), saleListings: a.saleListings + r.saleListings }; }, { properties: 0, units: 0, occupied: 0, available: 0, reserved: 0, tenants: 0, rent: 0, tax: 0, plots: 0, saleListings: 0 });

  return (
    <DashboardLayout navItems={adminNav} title="Dashboard">
      <div className="mb-7 rounded-2xl brand-gradient p-6 text-white shadow-soft-lg"><p className="text-sm font-semibold text-white/90">HighPark Consult administration</p><h2 className="mt-1 text-2xl font-bold text-white">Universal portfolio control centre</h2><p className="mt-2 max-w-2xl text-sm font-medium text-white/90">Monitor the universal portfolio, assets, occupancy, clients, verified collections, operations and compliance from one screen.</p></div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 mb-7">
        <StatCard label="Portfolio assets" value={totals.properties} icon={<Building2 className="w-5 h-5" />} onClick={() => navigate('/admin/properties')} />
        <StatCard label="Rentable spaces" value={totals.units} icon={<Home className="w-5 h-5" />} accent="accent" onClick={() => navigate('/admin/units')} />
        <StatCard label="Occupied spaces" value={totals.occupied} icon={<Users className="w-5 h-5" />} accent="blue" onClick={() => navigate('/admin/units?status=occupied')} />
        <StatCard label="Land / plots" value={totals.plots} icon={<Home className="w-5 h-5" />} accent="ink" onClick={() => navigate('/admin/properties')} />
        <StatCard label="Active occupants" value={totals.tenants} icon={<UserCheck className="w-5 h-5" />} onClick={() => navigate('/admin/users?role=customer&active=true')} />
        ...
        <StatCard label="Active sale listings" value={totals.saleListings} icon={<Receipt className="w-5 h-5" />} accent="accent" onClick={() => navigate('/admin/properties')} />
      ...
      <div className="mb-7 grid grid-cols-1 gap-4 md:grid-cols-2">
        <OperationalPreview title="Maintenance & service requests" description="Client-reported service issues requiring attention across the portfolio." href="/admin/maintenance" icon={<Wrench className="h-5 w-5" />} />
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-ink-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-semibold text-ink-900">Property performance</h3><p className="text-sm text-ink-500">Each asset is shown with metrics appropriate to its class and operating model.</p></div><span className="badge bg-brand-50 text-brand-700">Admin live view</span></div>
        <div className="grid grid-cols-1 gap-5 p-5 xl:grid-cols-2">{summary.map((r) => { const view = getPropertyPresentation(r.assetClass, r.operationModel, r.propertyType); return <button key={r.id} type="button" onClick={() => navigate(`/admin/properties/${r.id}`)} className="group rounded-2xl border border-ink-100 bg-white p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-soft-lg focus:outline-none focus:ring-2 focus:ring-brand-500/20"><div className="flex items-start justify-between gap-3"><div><h4 className="font-bold text-ink-900 group-hover:text-brand-700">{r.name}</h4><div className="mt-1 flex flex-wrap gap-1.5"><span className="badge bg-ink-100 text-ink-600">{r.propertyType}</span><span className="badge bg-brand-50 text-brand-700">{view.label}</span></div></div><span className="text-xs font-semibold text-brand-700">Inspect →</span></div>
          {view.showOccupancy && <div className="mt-4"><div className="mb-1 flex justify-between text-[11px] text-ink-500"><span>Occupancy</span><span className="font-semibold text-ink-700">{r.units ? Math.round(r.occupied / r.units * 100) : 0}%</span></div><div className="h-1.5 rounded-full bg-ink-100"><div className="h-1.5 rounded-full bg-brand-500" style={{width:`${r.units ? Math.round(r.occupied / r.units * 100) : 0}%`}}/></div></div>}
          {view.kind === 'land' ? <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-xl bg-accent-50 p-3"><p className="text-xl font-bold text-accent-700">{r.plotCount || 0}</p><p className="text-[10px] uppercase text-accent-600">Plots</p></div><div className="rounded-xl bg-ink-50 p-3"><p className="text-sm font-bold text-ink-900">{r.landArea ?? '—'}</p><p className="text-[10px] uppercase text-ink-400">{r.landAreaUnit || 'Area'}</p></div><div className="rounded-xl bg-ink-50 p-3"><p className="truncate text-sm font-bold text-ink-900">{r.plotDimensions || '—'}</p><p className="text-[10px] uppercase text-ink-400">Dimensions</p></div><div className="rounded-xl bg-brand-50 p-3"><p className="truncate text-sm font-bold text-brand-700">{r.parcelNumber || r.titleNumber || '—'}</p><p className="text-[10px] uppercase text-brand-600">Title / parcel</p></div></div> : view.kind === 'sale' || view.kind === 'development' ? <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl bg-ink-50 p-3"><p className="text-xl font-bold text-ink-900">{r.saleListings || 0}</p><p className="text-[10px] uppercase text-ink-400">Active sale listings</p></div><div className="rounded-xl bg-brand-50 p-3"><p className="text-sm font-bold text-brand-700">{r.saleMinPrice ? formatKES(r.saleMinPrice) : 'Enquire'}</p><p className="text-[10px] uppercase text-brand-600">Asking from</p></div></div> : view.kind === 'short_stay' ? <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl bg-ink-50 p-3"><p className="text-xl font-bold text-ink-900">{r.shortStayListings || 0}</p><p className="text-[10px] uppercase text-ink-400">Active listings</p></div><div className="rounded-xl bg-brand-50 p-3"><p className="text-sm font-bold text-brand-700">{r.shortStayMinRate ? `${formatKES(r.shortStayMinRate)}/night` : 'Enquire'}</p><p className="text-[10px] uppercase text-brand-600">From / night</p></div></div> : <div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-xl bg-ink-50 p-3"><p className="text-xl font-bold text-ink-900">{r.units}</p><p className="text-[10px] uppercase text-ink-400">{view.inventoryLabel}</p></div><div className="rounded-xl bg-brand-50 p-3"><p className="text-xl font-bold text-brand-700">{r.available}</p><p className="text-[10px] uppercase text-brand-600">{view.availableLabel}</p></div><div className="rounded-xl bg-accent-50 p-3"><p className="text-xl font-bold text-accent-700">{r.reserved}</p><p className="text-[10px] uppercase text-accent-600">{view.reservedLabel}</p></div></div>}
          {view.showTenantMetrics && <div className="mt-3 grid grid-cols-3 gap-3 border-t border-ink-100 pt-3 text-xs"><div><p className="text-ink-400">Tenants</p><p className="font-semibold">{r.tenants}</p></div><div><p className="text-ink-400">Verified rent</p><p className="font-semibold">{formatKES(r.collectedRent)}</p></div><div><p className="text-ink-400">Est. tax</p><p className="font-semibold text-brand-700">{formatKES(r.tax)}</p></div></div>}
        </button>})}</div>
      </Card>
      <div className="mt-7 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="p-6"><div className="mb-4"><h3 className="font-semibold text-ink-900">Actual unit / space mix</h3><p className="text-xs text-ink-500">Rentable inventory only; land and whole-asset records are excluded.</p></div><div className="grid grid-cols-2 gap-3">{Object.entries(unitMix).sort((a,b) => b[1].total-a[1].total).map(([type, x]) => <div key={type} className="rounded-xl border border-ink-100 bg-ink-50 p-3"><p className="text-sm font-semibold text-ink-800">{type}</p><p className="mt-1 text-2xl font-bold text-brand-700">{x.total}</p><p className="text-[11px] text-ink-500">{x.total === 1 ? '1 space' : `${x.total} spaces`} across the portfolio</p></div>)}{!Object.keys(unitMix).length && <p className="text-sm text-ink-500">No units yet.</p>}</div></Card>
...
export function AdminPropertyDetail({ propertyId }: { propertyId: string }) {
  ...
  const [reservations, setReservations] = useState<Array<Reservation & { profiles: { full_name: string } | null; property_units: { unit_number: string } | null }>>([]);
  const [leases, setLeases] = useState<Array<Lease & { profiles: { full_name: string } | null }>>([]);
  const [loading, setLoading] = useState(true);
    ...
    (async () => {
      ...
      try {
        const { data: propertyData, error: propertyError } = await supabase
          .from('properties').select('*').eq('id', propertyId).maybeSingle();
        if (propertyError) {
          toast(`Could not load property: ${propertyError.message}`, 'error');
          setProperty(null);
          setLoading(false);
          return;
        }
        ...
        const [unitsResult, reservationResult, leaseResult] = await Promise.all([
          supabase.from('property_units').select('*').eq('property_id', propertyId).order('unit_number', { ascending: true }),
          supabase.from('reservations').select('*, property_units(unit_number)').eq('property_id', propertyId).order('created_at', { ascending: false }).limit(8),
          supabase.from('leases').select('*').eq('property_id', propertyId).order('created_at', { ascending: false }).limit(8),
        ]);
        if (unitsResult.error) toast(`Units could not be loaded: ${unitsResult.error.message}`, 'error');
        if (reservationResult.error) toast(`Reservations could not be loaded: ${reservationResult.error.message}`, 'error');
        if (leaseResult.error) toast(`Leases could not be loaded: ${leaseResult.error.message}`, 'error');
        setUnits((unitsResult.data as PropertyUnit[]) || []);
        setReservations((reservationResult.data as typeof reservations) || []);
        setLeases((leaseResult.data as typeof leases) || []);
      } catch (error) {
        console.error('Admin property inspection error:', error);
        toast('Unable to open this property inspection right now.', 'error');
        setProperty(null);
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <DashboardLayout navItems={adminNav} title="Property inspection"><LoadingPage /></DashboardLayout>;
  if (!property) return <DashboardLayout navItems={adminNav} title="Property inspection"><EmptyState icon={<Building2 className="h-8 w-8" />} title="Property not found" description="This property may have been removed or is no longer accessible." action={<button className="btn-primary" onClick={() => navigate('/admin/properties')}>Back to registry</button>} /></DashboardLayout>;
  ...
  const heroStats = view.kind === 'land' ? [
    ['Plots', String(property.plot_count || 0)],
    ['Land area', `${property.total_land_area ?? '—'} ${property.land_area_unit || ''}`.trim()],
    ['Plot dimensions', property.plot_dimensions || '—'],
    ['Parcel / title', property.parcel_number || property.title_number || '—'],
  ] : view.kind === 'sale' || view.kind === 'development' ? [
    ['Asset mode', view.kind === 'sale' ? 'Sale' : 'Project'],
    ['Land area', property.total_land_area != null ? `${property.total_land_area} ${property.land_area_unit || ''}`.trim() : '—'],
    ['Agreements', String(leases.length)],
    ['Reservations', String(reservations.length)],
  ] : [
    [view.inventoryLabel, String(units.length)],
    ['Occupancy', `${occupancy}%`],
    ['Agreements', String(leases.length)],
    ['Bookings / reservations', String(reservations.length)],
  ];

  return <DashboardLayout navItems={adminNav} title="Property inspection">
    <div className="mb-5 flex items-center gap-2 text-sm"><button type="button" onClick={() => navigate('/admin/properties')} className="text-ink-500 hover:text-brand-700">Property registry</button><span className="text-ink-300">/</span><span className="font-semibold text-ink-800 truncate">{property.name}</span></div>
    <div className="overflow-hidden rounded-3xl border border-ink-100 bg-white shadow-soft-lg">
      <div className="relative h-56 sm:h-72 bg-ink-100">{image ? <img src={image} alt={property.name} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-ink-300"><Building2 className="h-16 w-16" /></div>}<div className="absolute inset-0 from-black/60 via-black/5 to-transparent" /><div className="absolute bottom-0 left-0 right-0 p-5 sm:p-7 text-white"><div className="flex flex-wrap gap-2 mb-2"><span className="badge bg-white/95 text-ink-800">{property.property_type}</span><Badge status={property.status} /></div><h1 className="text-2xl sm:text-3xl font-bold">{property.name}</h1><p className="mt-1 flex items-center gap-1.5 text-sm text-white/85"><MapPin className="h-4 w-4" />{property.estate ? `${property.estate}, ` : ''}{property.town}, {property.county}</p></div></div>
      <div className="grid grid-cols-2 gap-px bg-ink-100 sm:grid-cols-4">{heroStats.map(([label, value]) => <div key={label} className="bg-white p-4"><p className="text-2xl font-bold text-ink-900">{value}</p><p className="text-xs text-ink-500">{label}</p></div>)}</div>
    </div>
    {view.showOccupancy && <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="insight-card"><p className="text-xs text-ink-500">{view.occupiedLabel}</p><p className="mt-1 text-xl font-bold text-ink-900">{occupied}</p></div><div className="insight-card"><p className="text-xs text-ink-500">{view.availableLabel}</p><p className="mt-1 text-xl font-bold text-brand-700">{available}</p></div><div className="insight-card"><p className="text-xs text-ink-500">{view.reservedLabel}</p><p className="mt-1 text-xl font-bold text-ink-900">{reserved}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Maintenance</p><p className="mt-1 text-xl font-bold text-accent-700">{maintenance}</p></div></div>}
    <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <Card className="overflow-hidden"><div className="border-b border-ink-100 px-5 py-4"><h3 className="font-semibold text-ink-900">{view.showUnitInventory ? `${view.inventoryLabel} inventory` : 'Asset information'}</h3><p className="text-xs text-ink-500">{view.showUnitInventory ? 'Operational components belonging to this asset.' : 'This asset is managed as a whole rather than through residential unit inventory.'}</p></div>{view.kind === 'land' ? <div className="grid grid-cols-2 gap-3 p-5 sm:grid-cols-3"><div className="insight-card"><p className="text-xs text-ink-500">Plots</p><p className="mt-1 font-semibold text-ink-900">{property.plot_count || 0}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Dimensions</p><p className="mt-1 font-semibold text-ink-900">{property.plot_dimensions || 'Not provided'}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Zoning</p><p className="mt-1 font-semibold text-ink-900">{property.zoning || 'Not provided'}</p></div></div> : !view.showUnitInventory ? <div className="grid grid-cols-2 gap-3 p-5 sm:grid-cols-3"><div className="insight-card"><p className="text-xs text-ink-500">Asset class</p><p className="mt-1 font-semibold text-ink-900">{titleCase(property.asset_class || 'built_property')}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Operating model</p><p className="mt-1 font-semibold text-ink-900">{titleCase(property.operation_model || 'long_term_rental')}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Land area</p><p className="mt-1 font-semibold text-ink-900">{property.total_land_area ? `${property.total_land_area} ${property.land_area_unit || ''}` : '—'}</p></div></div> : units.length === 0 ? <EmptyState icon={<Home className="h-7 w-7" />} title={`No ${view.inventoryLabel.toLowerCase()} configured`} description="Add the operational components that belong to this asset." /> : <div className="overflow-x-auto"><table className="premium-table w-full min-w-[720px] text-sm"><thead><tr><th>Space / item</th><th>Type</th><th>Floor</th><th>Bedrooms / size</th><th>Rate</th><th>Status</th></tr></thead><tbody>{units.map((u) => <tr key={u.id}><td className="font-semibold text-ink-900">{u.unit_number}</td><td>{u.house_type || '—'}</td><td>{u.floor ?? '—'}</td><td>{u.bedrooms || 'Studio'}</td><td className="font-semibold">{formatKES(u.monthly_rent)}</td><td><Badge status={u.status} /></td></tr>)}</tbody></table></div>}</Card>
      <Card className="p-5"><h3 className="font-semibold text-ink-900">Property profile</h3><div className="mt-4 space-y-3 text-sm"><div><p className="text-xs text-ink-400">Asset class</p><p className="font-semibold text-ink-800">{titleCase(property.asset_class || 'built_property')}</p></div><div><p className="text-xs text-ink-400">Operation model</p><p className="font-semibold text-ink-800">{titleCase(property.operation_model || 'long_term_rental')}</p></div><div><p className="text-xs text-ink-400">Address</p><p className="font-semibold text-ink-800">{property.address || `${property.town}, ${property.county}`}</p></div><div><p className="text-xs text-ink-400">Primary asset details</p><p className="font-semibold text-ink-800">{view.kind === 'land' ? `${property.plot_count || 0} plots · ${property.plot_dimensions || 'dimensions not provided'}` : view.kind === 'short_stay' ? `${units.length} ${view.inventoryLabel.toLowerCase()} · hospitality operation` : `${property.number_of_units || 0} ${view.inventoryLabel.toLowerCase()} · ${property.number_of_floors || 0} floors`}</p></div></div>{view.showUnitInventory && <div className="mt-5 grid grid-cols-2 gap-2"><button className="btn-secondary text-sm" onClick={() => navigate(`/admin/units?property=${property.id}`)}><Home className="h-4 w-4" /> {view.inventoryLabel}</button><button className="btn-secondary text-sm" onClick={() => navigate(`/admin/reservations?property=${property.id}`)}><Calendar className="h-4 w-4" /> Reservations</button></div>}</Card>
    </div>
    {view.showOccupancy && <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"><Card className="overflow-hidden"><div className="border-b border-ink-100 px-5 py-4"><h3 className="font-semibold text-ink-900">Recent reservations</h3></div>{reservations.length === 0 ? <p className="p-5 text-sm text-ink-500">No reservations for this property.</p> : <div className="divide-y divide-ink-100">{reservations.map((r) => <div key={r.id} className="flex items-center justify-between gap-3 p-4"><div><p className="font-semibold text-ink-800">{r.customer_id ? 'Customer reservation' : 'Reservation'}</p><p className="text-xs text-ink-500">Unit {r.property_units?.unit_number || '—'} · {formatDate(r.created_at)}</p></div><Badge status={r.status} /></div>)}</div>}</Card><Card className="overflow-hidden"><div className="border-b border-ink-100 px-5 py-4"><h3 className="font-semibold text-ink-900">Recent leases</h3></div>{leases.length === 0 ? <p className="p-5 text-sm text-ink-500">No leases for this property.</p> : <div className="divide-y divide-ink-100">{leases.map((l) => <div key={l.id} className="flex items-center justify-between gap-3 p-4"><div><p className="font-semibold text-ink-800">{l.tenant_id ? 'Tenant lease' : 'Lease'}</p><p className="text-xs text-ink-500">{formatDate(l.lease_start)} → {formatDate(l.lease_end)}</p></div><Badge status={l.status} /></div>)}</div>}</Card></div>}
  </DashboardLayout>;
}
        ...
        <Card className="overflow-hidden">
          ...
          <div className="overflow-x-auto"><table className="premium-table w-full min-w-[980px] text-sm"><thead><tr><th>Property</th><th>Owner</th><th>Location</th><th>Type</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            {properties.map((p) => <tr key={p.id} className={selectedProperty === p.id ? 'bg-brand-50/70' : ''}>
              <td><button type="button" className="text-left" onClick={() => navigate(`/admin/properties/${p.id}`)}><p className="font-semibold text-ink-900 hover:text-brand-700">{p.name}</p><p className="mt-0.5 text-xs text-ink-400">{(() => { const view = getPropertyPresentation(p.asset_class, p.operation_model, p.property_type); return view.kind === 'land' ? `${p.plot_count || 0} plots · ${p.total_land_area || '—'} ${p.land_area_unit || ''}`.trim() : view.kind === 'short_stay' ? 'Hospitality / short-stay asset' : view.kind === 'sale' ? 'Sale asset' : view.kind === 'development' ? `${p.total_land_area || '—'} ${p.land_area_unit || ''} · development project`.trim() : `${p.number_of_units || 0} ${view.inventoryLabel.toLowerCase()} · ${p.number_of_floors || 0} floors`; })()}</p></button></td>
...
function AdminFinancialCommandCenter({ data }: { data: AdminFinancialData }) {
  ...
  const topProperties = [...data.properties].filter((p) => p.units > 0).sort((a,b) => b.net-a.net).slice(0,6);
...
export function AdminExpenses() {
  ...
  return <DashboardLayout navItems={adminNav} title="Expenses">
    ...
    <div className="mb-6 grid grid-cols-1 gap-6 xl:grid-cols-2"><DonutChart segments={Object.entries(rows.reduce((acc, e) => { const key = String(e.category || 'Other'); acc[key] = (acc[key] || 0) + Number(e.amount || 0); return acc; }, {} as Record<string, number>)).map(([label, value]) => ({ label, value: Math.round(value) }))} centerLabel="Recent" centerValue={formatKES(rows.reduce((sum, e) => sum + Number(e.amount || 0), 0))} /><TrendChart points={rows.slice().reverse().map(e => ({ label: new Date(String(e.expense_date)).toLocaleDateString(undefined,{day:'2-digit',month:'short'}), value:Number(e.amount||0) })).slice(-6)} valueLabel="Recent expense activity" prefix="KES " /></div>{loading ? <LoadingPage /> : rows.length === 0 ? <EmptyState icon={<Receipt className="h-8 w-8" />} title="No expenses recorded" description="Expenses recorded by owners or administrators will appear here." /> : <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="premium-table w-full min-w-[900px] text-sm"><thead><tr><th>Property</th><th>Category</th><th>Amount</th><th>Date</th><th>Vendor</th><th>Method</th></tr></thead><tbody>{rows.map((e) => <tr key={String(e.id)} className="hover:bg-ink-50"><td className="px-4 py-4 font-semibold text-ink-900">{e.properties?.name || '—'}</td><td className="px-4 py-4">{e.category}</td><td className="px-4 py-4 font-bold">{formatKES(Number(e.amount || 0))}</td><td className="px-4 py-4 text-ink-500">{formatDate(String(e.expense_date))}</td><td className="px-4 py-4">{e.vendor || '—'}</td><td className="px-4 py-4 capitalize">{e.payment_method || 'cash'}</td></tr>)}</tbody></table></div></Card>}
        <Pagination page={page} totalPages={totalPages} onPageChange={setPage} />
</DashboardLayout>;
}
...
export function AdminLeases() {
  ...
  return <DashboardLayout navItems={adminNav} title="Agreements & Occupants">
    <AdminPageHeader eyebrow="Occupant & agreement lifecycle" title="Agreements & Occupants" description="Control the full occupant and agreement lifecycle: active leases, expiry risk, outstanding balances, renewals, move-outs and asset relationships." action={<button type="button" onClick={() => void runAction('expire')} disabled={busy} className="btn-secondary"><CalendarClock className="h-4 w-4" /> Process expired leases</button>} />
    <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
