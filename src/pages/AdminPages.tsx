...
export function AdminPropertyDetail({ propertyId }: { propertyId: string }) {
  ...
  return <DashboardLayout navItems={adminNav} title="Property inspection">
    ...
    <div className="overflow-hidden rounded-3xl border border-ink-100 bg-white shadow-soft-lg">
      ...
      <div className="grid grid-cols-2 gap-px bg-ink-100 sm:grid-cols-4"><div className="bg-white p-4"><p className="text-2xl font-bold text-ink-900">{units.length}</p><p className="text-xs text-ink-500">Units / inventory</p></div><div className="bg-white p-4"><p className="text-2xl font-bold text-brand-700">{occupancy}%</p><p className="text-xs text-ink-500">Occupancy</p></div><div className="bg-white p-4"><p className="text-2xl font-bold text-ink-900">{leases.length}</p><p className="text-xs text-ink-500">Agreements</p></div><div className="bg-white p-4"><p className="text-2xl font-bold text-ink-900">{reservations.length}</p><p className="text-xs text-ink-500">Bookings / reservations</p></div></div>
    </div>
    <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="insight-card"><p className="text-xs text-ink-500">Occupied</p><p className="mt-1 text-xl font-bold text-ink-900">{occupied}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Available</p><p className="mt-1 text-xl font-bold text-brand-700">{available}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Reserved</p><p className="mt-1 text-xl font-bold text-ink-900">{reserved}</p></div><div className="insight-card"><p className="text-xs text-ink-500">Maintenance</p><p className="mt-1 text-xl font-bold text-accent-700">{maintenance}</p></div></div>
    <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <Card className="overflow-hidden"><div className="border-b border-ink-100 px-5 py-4"><h3 className="font-semibold text-ink-900">Asset inventory</h3><p className="text-xs text-ink-500">Inventory and operational components belonging to this asset.</p></div>{units.length === 0 ? <EmptyState icon={<Home className="h-7 w-7" />} title="No units configured" description="Add units from the property management workflow." /> : <div className="overflow-x-auto"><table className="premium-table w-full min-w-[720px] text-sm"><thead><tr><th>Unit / item</th><th>Type</th><th>Floor</th><th>Bedrooms / size</th><th>Rate</th><th>Status</th></tr></thead><tbody>{units.map((u) => <tr key={u.id}><td className="font-semibold text-ink-900">{u.unit_number}</td><td>{u.house_type || '—'}</td><td>{u.floor ?? '—'}</td><td>{u.bedrooms || 'Studio'}</td><td className="font-semibold">{formatKES(u.monthly_rent)}</td><td><Badge status={u.status} /></td></tr>)}</tbody></table></div>}</Card>
      <Card className="p-5"><h3 className="font-semibold text-ink-900">Property profile</h3><div className="mt-4 space-y-3 text-sm"><div><p className="text-xs text-ink-400">Asset class</p><p className="font-semibold text-ink-800">{titleCase(property.asset_class || 'built_property')}</p></div><div><p className="text-xs text-ink-400">Operation model</p><p className="font-semibold text-ink-800">{titleCase(property.operation_model || 'long_term_rental')}</p></div><div><p className="text-xs text-ink-400">Address</p><p className="font-semibold text-ink-800">{property.address || `${property.town}, ${property.county}`}</p></div><div><p className="text-xs text-ink-400">Inventory / floors</p><p className="font-semibold text-ink-800">{property.number_of_units || 0} units · {property.number_of_floors || 0} floors</p></div></div><div className="mt-5 grid grid-cols-2 gap-2"><button className="btn-secondary text-sm" onClick={() => navigate(`/admin/units?property=${property.id}`)}><Home className="h-4 w-4" /> Units</button><button className="btn-secondary text-sm" onClick={() => navigate(`/admin/reservations?property=${property.id}`)}><Calendar className="h-4 w-4" /> Reservations</button></div></Card>
    </div>
    <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"><Card className="overflow-hidden"><div className="border-b border-ink-100 px-5 py-4"><h3 className="font-semibold text-ink-900">Recent reservations</h3></div>{reservations.length === 0 ? <p className="p-5 text-sm text-ink-500">No reservations for this property.</p> : <div className="divide-y divide-ink-100">{reservations.map((r) => <div key={r.id} className="flex items-center justify-between gap-3 p-4"><div><p className="font-semibold text-ink-800">{r.customer_id ? 'Customer reservation' : 'Reservation'}</p><p className="text-xs text-ink-500">Unit {r.property_units?.unit_number || '—'} · {formatDate(r.created_at)}</p></div><Badge status={r.status} /></div>)}</div>}</Card><Card className="overflow-hidden"><div className="border-b border-ink-100 px-5 py-4"><h3 className="font-semibold text-ink-900">Recent leases</h3></div>{leases.length === 0 ? <p className="p-5 text-sm text-ink-500">No leases for this property.</p> : <div className="divide-y divide-ink-100">{leases.map((l) => <div key={l.id} className="flex items-center justify-between gap-3 p-4"><div><p className="font-semibold text-ink-800">{l.tenant_id ? 'Tenant lease' : 'Lease'}</p><p className="text-xs text-ink-500">{formatDate(l.lease_start)} → {formatDate(l.lease_end)}</p></div><Badge status={l.status} /></div>)}</div>}</Card></div>
  </DashboardLayout>;
}
        ...
        <Card className="overflow-hidden">
          ...
          <div className="overflow-x-auto"><table className="premium-table w-full min-w-[980px] text-sm"><thead><tr><th>Property</th><th>Owner</th><th>Location</th><th>Type</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            {properties.map((p) => <tr key={p.id} className={selectedProperty === p.id ? 'bg-brand-50/70' : ''}>
              <td><button type="button" className="text-left" onClick={() => navigate(`/admin/properties/${p.id}`)}><p className="font-semibold text-ink-900 hover:text-brand-700">{p.name}</p><p className="mt-0.5 text-xs text-ink-400">{p.number_of_units || 0} listed units · {p.number_of_floors || 0} floors</p></button></td>
