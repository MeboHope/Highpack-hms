...
export function AdminPropertyDetail({ propertyId }: { propertyId: string }) {
  ...
  return <DashboardLayout navItems={adminNav} title="Property inspection">
    ...
    <div className="overflow-hidden rounded-3xl border border-ink-100 bg-white shadow-soft-lg">
      <div className="relative h-56 sm:h-72 bg-ink-100">{image ? <img src={image} alt={property.name} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-ink-300"><Building2 className="h-16 w-16" /></div>}<div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/5 to-transparent" /><div className="absolute bottom-0 left-0 right-0 p-5 sm:p-7 text-white"><div className="flex flex-wrap gap-2 mb-2"><span className="badge bg-white/95 text-ink-800">{property.property_type}</span><Badge status={property.status} /></div><h1 className="text-2xl sm:text-3xl font-bold">{property.name}</h1><p className="mt-1 flex items-center gap-1.5 text-sm text-white/85"><MapPin className="h-4 w-4" />{property.estate ? `${property.estate}, ` : ''}{property.town}, {property.county}</p></div></div>
